...
try {
    ...
    if (!$pesan) {
        echo "<script>alert('Pesan tidak ditemukan!'); window.location.href='../admin_panel/broadcast.php';</script>";
        exit();
    }
...
<head>
    ...
    <title>Edit Broadcast - Admin PKM</title>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white border border-slate-200 rounded-t-3xl p-6 md:p-8 border-b-0">
            <div class="flex items-center gap-4">
                <!-- Warna diubah ke Amber untuk mode EDIT -->
                <div class="bg-amber-500 w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-200">
                    <i class="fas fa-edit text-white text-xl"></i>
                </div>
                <div>
                    <h1 class="text-slate-800 font-black text-2xl tracking-tighter uppercase italic">Koreksi Broadcast</h1>
                    <p class="text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em] mt-0.5">Perbarui isi pengumuman yang sudah terkirim</p>
                </div>
            </div>
        </div>

        <!-- Form Body -->
        <form action="../proses_edit/proses_edit_broadcast.php" method="POST" class="space-y-0">
            ...
            <div class="bg-white p-6 md:p-8 border-x border-slate-200 space-y-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            <input type="text" name="judul_pesan" 
                                   value="<?= htmlspecialchars($pesan['judul_pesan']) ?>"
                                   placeholder="Contoh: Pengingat Batas Proposal" 
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            <select name="tujuan_pesan" 
                                    class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 pl-10 outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition font-bold text-slate-700 shadow-sm appearance-none cursor-pointer" required>
                                <option value="Semua" <?= $pesan['tujuan_pesan'] == 'Semua' ? 'selected' : '' ?>>Semua Civitas</option>
                                <option value="Mahasiswa" <?= $pesan['tujuan_pesan'] == 'Mahasiswa' ? 'selected' : '' ?>>Khusus Mahasiswa</option>
                                <option value="Dosen" <?= $pesan['tujuan_pesan'] == 'Dosen' ? 'selected' : '' ?>>Khusus Dosen</option>
                            </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="isi_pesan" rows="8" placeholder="Tuliskan detail pengumuman di sini..." 
                              class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition text-slate-600 shadow-sm leading-relaxed" required><?= htmlspecialchars($pesan['isi_pesan']) ?></textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-6 md:p-8 rounded-b-3xl border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <button type="submit" class="w-full md:w-auto px-10 py-3 bg-amber-600 text-white font-black text-[10px] uppercase tracking-[0.2em] rounded-xl shadow-xl shadow-amber-900/20 hover:bg-amber-700 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                    <i class="fas fa-save"></i> Perbarui & Simpan
                </button>
            </div>
        </form>
    </div>
