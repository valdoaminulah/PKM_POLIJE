...
try {
    // 1. Ambil Info Tim & Data Ketua (Owner)
    ...
    // Keamanan: Hanya ketua tim yang boleh mengakses halaman "Manajemen" ini
    if (!$data_tim || $data_tim['id_ketua'] != $id_user_login) {
        echo "<script>alert('Akses Ditolak! Anda bukan ketua tim ini.'); window.location='../buat_tim/daftar_tim.php';</script>";
        exit;
    }

    // 2. Ambil Daftar Anggota yang sudah DITERIMA
    $stmt_anggota = $pdo->prepare("SELECT p.id_pendaftaran, u.nama_lengkap, u.nim, u.angkatan, u.program_studi, u.no_whatsapp 
...
<body class="bg-slate-50">
   ...
   <main class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center space-x-4 mb-8">
        <a href="../buat_tim/daftar_tim.php" class="p-2 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition">
        ...
        <div>
            ...
            <p class="text-gray-500">Kelola kolaborator untuk tim <span class="text-blue-600 font-semibold"><?php echo htmlspecialchars($data_tim['nama_tim']); ?></span></p>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-gray-100">
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-gray-400">Program Studi / Kontak</th>
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-gray-400 text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    
                    <tr class="hover:bg-slate-50/50 transition bg-blue-50/20">
                        <td class="px-6 py-5">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-gray-800"><?php echo htmlspecialchars($data_tim['nama_lengkap']); ?></p>
                                    <span class="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-md font-bold uppercase">Ketua / Anda</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-5">
                            <p class="text-sm text-gray-600 font-medium"><?php echo $data_tim['nim']; ?></p>
                            <p class="text-xs text-gray-400 font-bold uppercase tracking-tighter">Angkatan <?php echo $data_tim['angkatan']; ?></p>
                        </td>
                        <td class="px-6 py-5 leading-relaxed">
                            <p class="text-sm text-gray-700 font-semibold"><?php echo $data_tim['program_studi']; ?></p>
                            <a href="https://wa.me/<?php echo $data_tim['no_whatsapp']; ?>" target="_blank" class="text-xs text-green-500 font-bold hover:underline flex items-center mt-1">
                                <?php echo $data_tim['no_whatsapp']; ?>
                            </a>
                        </td>
                        <td class="px-6 py-5 text-center">
                            <span class="text-gray-300 text-xs italic font-medium tracking-tight">Owner</span>
                        </td>
                    </tr>

                    <?php if (count($list_anggota) > 0): ?>
                        <?php foreach ($list_anggota as $anggota): ?>
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-6 py-5">
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 font-bold text-sm">
                                        <?php echo strtoupper(substr($anggota['nama_lengkap'], 0, 2)); ?>
                                    </div>
                                    <div>
                                        <p class="text-sm font-bold text-gray-800"><?php echo htmlspecialchars($anggota['nama_lengkap']); ?></p>
                                        <span class="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-md font-bold uppercase">Anggota</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <p class="text-sm text-gray-600 font-medium"><?php echo $anggota['nim']; ?></p>
                                <p class="text-xs text-gray-400 font-bold uppercase tracking-tighter">Angkatan <?php echo $anggota['angkatan']; ?></p>
                            </td>
                            <td class="px-6 py-5 leading-relaxed">
                                <p class="text-sm text-gray-700 font-semibold"><?php echo $anggota['program_studi']; ?></p>
                                <a href="https://wa.me/<?php echo $anggota['no_whatsapp']; ?>" target="_blank" class="text-xs text-green-500 font-bold hover:underline flex items-center mt-1">
                                    <?php echo $anggota['no_whatsapp']; ?>
                                </a>
                            </td>
                            <td class="px-6 py-5 text-center">
                                <button onclick="confirmKick('<?php echo $anggota['id_pendaftaran']; ?>', '<?php echo addslashes($anggota['nama_lengkap']); ?>')" 
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-gray-400 italic">Belum ada anggota yang bergabung.</td>
                        </tr>
                    <?php endif; ?>
...
<script>
// Gabungkan Navbar Toggle dan Fungsi Kick di sini agar rapi
...
// HANYA GUNAKAN SATU FUNGSI INI
function confirmKick(idPendaftaran, nama) {
    if (confirm(`Apakah Anda yakin ingin mengeluarkan ${nama} dari tim? Tindakan ini akan menghapus status keanggotaannya secara permanen.`)) {
        // Mengarahkan ke file proses dengan parameter yang lengkap
        window.location.href = '../proses/proses_kick_anggota.php?id_p=' + idPendaftaran + '&id_t=<?php echo $id_tim; ?>';
    }
}
</script>
</body>
